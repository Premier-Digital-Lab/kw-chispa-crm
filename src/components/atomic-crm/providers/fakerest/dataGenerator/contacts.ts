...
export const generateContacts = (db: Db, size = 500): Required<Contact>[] => {
  ...
  return Array.from(Array(size).keys()).map((id) => {
    ...
    return {
      ...
      nb_tasks: 0,
      linkedin_url: null,
      // KW CHISPA member directory fields — sensible demo defaults
      is_chapter_leader: false,
      cell_number: phone.phoneNumber(),
      market_center_name: `${fakerCompany.companyName()} Market Center`,
      market_center_team_leader: null,
      market_center_tl_phone: null,
      market_center_tl_email: null,
      agent_role: random.arrayElement(["Agent", "Team Leader", "Broker"]),
      mc_street_address: null,
      mc_suite_unit: null,
      mc_city: null,
      mc_state: null,
      mc_zip_code: null,
      ...
      states_served: [],
      countries_served: [],
      membership_tier: random.arrayElement(["Free", "Premier"]),
      join_date: first_seen,
