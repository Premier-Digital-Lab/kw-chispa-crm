-- Migration: Split market_center_address into individual address components
--
-- market_center_address (text) is replaced by seven granular fields to
-- support international address formats for the KW CHISPA member directory.
--
-- Steps:
--   1. Drop contacts_summary (uses co.* — expanded at CREATE VIEW time,
--      so it holds a reference to market_center_address that would break
--      after the column drop)
--   2. Drop market_center_address column
--   3. Add mc_street_number, mc_street_name, mc_suite_unit, mc_city,
--      mc_state, mc_zip_code, mc_country
--   4. Recreate contacts_summary (same definition as previous migration,
--      co.* now expands to include the new mc_* columns)
--   5. Recreate merge_contacts() replacing market_center_address with the
--      seven new component fields
...
-- ============================================================
-- Step 2: Drop the old single-field column
-- ============================================================

ALTER TABLE public.contacts
    DROP COLUMN IF EXISTS market_center_address;


-- ============================================================
-- Step 3: Add individual Market Center address component columns
-- ============================================================

ALTER TABLE public.contacts
    ADD COLUMN mc_street_number  text,
    ADD COLUMN mc_street_name    text,
    ADD COLUMN mc_suite_unit     text,
    ADD COLUMN mc_city           text,
    ADD COLUMN mc_state          text,
    ADD COLUMN mc_zip_code       text,
    ADD COLUMN mc_country        text;


-- ============================================================
-- Step 4: Recreate contacts_summary
--
-- Identical to the definition from 20260409000000_members_directory_fields.sql.
-- co.* now expands to include the new mc_* columns (and excludes the
-- dropped market_center_address column).
-- ============================================================
...
-- ============================================================
-- Step 5: Recreate merge_contacts()
--
-- Replaces the single market_center_address COALESCE with individual
-- COALESCE calls for each of the seven new address component columns.
-- All other logic is unchanged from 20260409000000_members_directory_fields.sql.
-- ============================================================
...
BEGIN
  ...
  UPDATE contacts SET
    ...
    -- Market Center address components (prefer winner's values)
