-- Fix handle_new_sales_member(): the previous version queried auth.users with
-- WHERE id = NEW.id (bigint), but auth.users.id is a UUID. The correct join
-- column is NEW.user_id, which is the uuid FK on public.sales.
...
BEGIN
  ...
  -- Read user metadata passed in options.data during supabase.auth.signUp().
  -- NEW.user_id is the UUID FK that links public.sales to auth.users.
